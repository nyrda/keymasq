...
The status bar at the bottom shows the connection state. Wait until every indicator is green before you continue.
...
The device tab opens and shows every button on the device.

![Device tab with button grid](assets/screenshots/keymasq_device_fresh.png)

## Remap a key

This example remaps Caps Lock to Escape.
...
The key now shows its new mapping. Caps Lock now sends Escape in every app.
