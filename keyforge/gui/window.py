...
log = logging.getLogger("keyforge.gui.window")
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _on_recording_stopped(self, event: dict) -> None:
        from keyforge.gui.widgets.save_macro_dialog import SaveMacroDialog
    ...
    def _update_status_from_keyforged_event(self, keyforged_connected: bool) -> None:
        if keyforged_connected:
            self.session_status.set_label("session: 🟢")
            self.keyforged_status.set_label("keyforged: 🟢")
            self._set_connection_issue(None)
        else:
            self.session_status.set_label("session: 🟡")
            self.keyforged_status.set_label("keyforged: 🔴")
            self._set_connection_issue("keyforged")
    ...
    def _set_connection_issue(self, issue: str | None) -> None:
        ...
        if issue == "session":
            title = "Cannot Reach keyforge-session"
            body = (
                "The GUI is disconnected from keyforge-session.\n\n"
                "Start or restart the user service and try again:\n"
                "systemctl --user restart keyforge-session"
            )
        else:
            title = "keyforged Is Not Connected"
            body = (
                "keyforge-session is running, but it cannot reach keyforged.\n\n"
    ...
    def _setup_content(self) -> None:
        ...
        from keyforge.gui.widgets.recording_overlay import RecordingOverlay
        ...
        self.keyforged_status = Gtk.Label(label="keyforged: ⚪")
        self.keyforged_status.add_css_class("caption")
        self.keyforged_status.set_tooltip_text(
            "keyforged status (via session):\n"
            "🟢 Running\n"
            "🔴 Not running\n"
            "⚪ Unknown (session not connected)"
        )
        self.status_bar.append(self.keyforged_status)

        self.session_status = Gtk.Label(label="session: ⚪")
        self.session_status.add_css_class("caption")
        self.session_status.set_tooltip_text(
            "keyforge-session status:\n"
            "🟢 Running and connected to keyforged\n"
            "🟡 Running but NOT connected to keyforged\n"
            "🔴 Not running"
        )
    ...
    def _on_confirm_unlock_clicked(
        ...
        def worker() -> tuple[str, dict | None]:
            ...
            uid = os.getuid()
            helper_path = resolve_keyforge_record_helper_path()
            if helper_path is None:
                return f"Recording helper not found at {KEYFORGE_RECORD_HELPER_PATH}", None
