...
class WlrForeignToplevelManagerTracker:
...
def _state_has_activated(state: object) -> bool:
    ...
    if isinstance(state, bytes):
        return _decode_wlr_state_bytes(state)
    if isinstance(state, str):
        normalized = state.strip().lower()
        return normalized == "activated" or "activated" in normalized
    if isinstance(state, int):
        return state == WLR_TOPLEVEL_STATE_ACTIVATED
    return False


def _decode_wlr_state_bytes(state_bytes: bytes) -> bool:
    if not state_bytes:
        return False
    ...
    for idx in range(0, len(state_bytes), unit_size):
        ...
        if value == WLR_TOPLEVEL_STATE_ACTIVATED:
            return True
    return False
