...
class CatalogControllerMixin:
    ...
    def _append_saved_macro_actions(self, box: Gtk.Box, name: str) -> None:
        ...
        edit_btn.set_tooltip_text("Edit macro (Shift-click to open in a new window)")
        edit_btn.add_css_class("flat")
        edit_btn.connect("clicked", self._on_edit_clicked, name)
        edit_gesture = Gtk.GestureClick(button=1)
        edit_gesture.set_propagation_phase(Gtk.PropagationPhase.CAPTURE)
        edit_gesture.connect("pressed", self._on_edit_pressed)
        edit_gesture.connect("released", self._on_edit_released)
        edit_btn.add_controller(edit_gesture)
        edit_keys = Gtk.EventControllerKey()
        edit_keys.set_propagation_phase(Gtk.PropagationPhase.CAPTURE)
        edit_keys.connect("key-pressed", self._on_edit_key_pressed)
        edit_btn.add_controller(edit_keys)
        box.append(edit_btn)
