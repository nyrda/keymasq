...
def _start_startup_probe(window) -> None:
    if window.demo_mode:
        _runtime.GLib.idle_add(
            window._on_startup_probe_finished,
            _runtime.GuiTaskResult(value=window._probe_startup_state()),
        )
        return

    _runtime.run_gui_task(
        window._probe_startup_state,
        window._on_startup_probe_finished,
    )
...
def _on_startup_probe_finished(
    ...
    window._apply_compositor_state(compositor_state)
...
def _apply_compositor_state(window, state: dict[str, object]) -> None:
    ...
    window._maybe_present_gnome_setup_dialog()
...
def _update_compositor_status(window) -> None:
    ...
    if window._compositor_supported:
        ...
        if window._compositor_id == "gnome" and window._gnome_setup_needed():
