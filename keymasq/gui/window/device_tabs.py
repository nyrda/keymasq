...
def _device_tab_title(window, device: HardwareConfig) -> str:
    if window.demo_mode:
        return device.name
    status = window._device_status_for_hardware_id(device.hardware_id)
...
def _refresh_device_tabs(
    ...
    for child in window._iter_profile_tabs():
...
def _ensure_placeholder_page(window) -> None:
    if window._page_for_child(window.placeholder) is not None:
        return
    ...
    window._placeholder_page = window._append_tab_page(
        window.placeholder,
        title="Welcome",
...
def _apply_loaded_devices(window, devices: list[HardwareConfig]) -> None:
    if window.demo_mode and not devices:
        window._load_demo_devices()
        return

    if not devices:
        window._set_empty_placeholder_state()
        window._select_saved_or_default_tab()
        return

    window._close_tab_page(window._page_for_child(window.placeholder))
    ...
    try:
        for device in window._order_devices_for_tabs(devices):
            if device.hardware_id in window._device_pages:
                continue
            window._add_device_tab(device, persist_order=False)
        window._reorder_visible_pages_to_saved_order()
    finally:
...
def _load_demo_devices(window) -> None:
    ...
    try:
        if demo_device.hardware_id not in window._device_pages:
            window._add_device_tab(demo_device, persist_order=False)
        window._reorder_visible_pages_to_saved_order()
    finally:
        window._suppress_selected_tab_save = suppress_selected_was
    window._select_saved_or_default_tab()
...
def _ensure_combo_tab_page(window) -> _runtime.Adw.TabPage | None:
    ...
    window._combo_page = window._append_tab_page(
        window.combo_tab,
        title="Combos",
    ...
    window._apply_profile_runtime_state_to_widget(window.combo_tab)
    window._reorder_visible_pages_to_saved_order()
    return window._combo_page
...
def show_combo_tab(window) -> None:
    ...
    page = window._ensure_combo_tab_page()
    if page is None:
        return
    window._save_tab_layout()
...
def _add_device_tab(window, device: HardwareConfig, *, persist_order: bool = True) -> None:
    if window._placeholder_page is not None:
        window._close_tab_page(window._placeholder_page)
        window._placeholder_page = None
    ...
    page = window._append_tab_page(tab, title=device.name, icon_name=icon)
    ...
    window._apply_profile_runtime_state_to_widget(tab)
    ...
    window._reorder_visible_pages_to_saved_order()
    window._sync_device_tab_title(device.hardware_id)
    if persist_order:
        window._save_tab_layout()
...
def _on_add_device(window, button: _runtime.Gtk.Button) -> None:
    if window.demo_mode:
        window._show_demo_notification("Device setup not available in demo mode")
        return
    ...
    dialog.connect("device-created", window._on_device_created)
    dialog.present(window)


def _on_add_device_clicked(window, _button: _runtime.Gtk.Button) -> None:
    window._on_add_device(_button)
