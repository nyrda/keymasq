...
def open_device_inspector(window, device) -> None:
    ...
    if window.demo_mode:
        window._show_demo_notification("Device inspector is not available in demo mode")
        return

    if not window._device_inspector_unlock_ready():

        def reopen_inspector() -> None:
            window.open_device_inspector(device)

        window.present_unlock_dialog(on_success=reopen_inspector)
        return
...
def open_combo_inspector(window) -> None:
    ...
    if window.demo_mode:
        window._show_demo_notification("Combo inspector is not available in demo mode")
        return
