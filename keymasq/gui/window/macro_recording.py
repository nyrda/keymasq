...
def _on_recording_stopped(window, event: dict) -> None:
    window.present_pending_macro_save_dialog(event)
...
def present_pending_macro_save_dialog(window, recording_data: dict | None = None) -> bool:
    ...
    dialog.connect("closed", window._on_save_macro_dialog_closed)
...
def _refresh_macro_recording_state_from_session(
    ...
    def _on_status(status: dict | None) -> bool:
        ...
        if status_data is not None:
            window._update_macro_recording_state(status_data)
...
def present_macro_recording_enable_dialog(window, on_success=None) -> None:
    def _after_refresh(_status: dict | None) -> None:
        ...
        dialog.connect("response", window._on_macro_recording_enable_response, on_success)
        dialog.present(window)

    window._refresh_macro_recording_state_from_session(_after_refresh)
...
def present_macro_recording_disable_dialog(window, on_success=None) -> None:
    def _after_refresh(_status: dict | None) -> None:
        ...
        dialog.connect("response", window._on_macro_recording_disable_response, on_success)
        dialog.present(window)
...
def _on_macro_recording_enable_response(
    ...
    if response != "enable":
        return
    window._start_macro_recording_enable(on_success=on_success)
...
def _on_macro_recording_disable_response(
    ...
    if response != "disable":
        return
    window._start_macro_recording_disable(on_success=on_success)
...
def _start_macro_recording_enable(window, on_success=None) -> None:
    ...
    if window.demo_mode:
        window._show_unlock_error_dialog("Macro recording opt-in is unavailable in demo mode.")
        return
    ...
    def on_worker_done(result: _runtime.GuiTaskResult[tuple[str, dict | None]]) -> bool:
        ...
        return window._on_macro_recording_enable_finished(
            success,
            error_msg,
            on_success,
            status,
...
def _start_macro_recording_disable(window, on_success=None) -> None:
    ...
    if window.demo_mode:
        window._show_unlock_error_dialog("Macro recording opt-out is unavailable in demo mode.")
        return
...
def _on_macro_recording_enable_finished(
    ...
    if success:
        window._update_macro_recording_state(status)
...
def present_recording_settings_dialog(window, reason: str = "settings") -> None:
    ...
    dialog.connect("closed", window._on_record_macro_dialog_closed)
