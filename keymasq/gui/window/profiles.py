...
def _queue_profile_reload(window) -> None:
    ...
    _runtime.run_gui_task(
        window._load_profile_manager_snapshot,
        window._on_profile_reload_finished,
    )
...
def _on_profile_reload_finished(window, result: _runtime.GuiTaskResult[ProfileManager]) -> bool:
    ...
    if not window._destroyed and result.ok and isinstance(result.value, ProfileManager):
        window._set_profile_manager(result.value)
        window._refresh_device_tabs()
        window._apply_profile_runtime_state(window._profile_runtime_state)
...
def _apply_profile_runtime_state(window, state: dict | None) -> None:
    window._profile_runtime_state = window._normalize_profile_runtime_state(state)
    for child in window._iter_profile_tabs():
        window._apply_profile_runtime_state_to_widget(child)
    window._sync_device_tab_titles()
...
def _mark_device_runtime_unknown(window) -> None:
    ...
    for hardware_id, raw_device in devices.items():
        ...
        device_payload["device_status"] = {
            "state": "unknown",
            "configured_count": window._runtime_status_int(old_status, "configured_count"),
            "connected_count": 0,
            "requested_count": window._runtime_status_int(old_status, "requested_count"),
            "grabbed_count": 0,
            "interfaces": [],
            "runtime_ready": False,
            "grab_status": {},
    ...
    if updated_devices:
        window._apply_profile_runtime_state({"devices": updated_devices})
