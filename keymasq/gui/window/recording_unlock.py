...
def _update_unlock_state(window, status_data: dict | None) -> None:
    ...
    if not window._recording_unlocked:
        ...
        if window._recording_claim_attempt_key != claim_key:
            ...
            _request_recording_refresh_lease(window)

    _refresh_macro_menu_state(window)
    _refresh_unlock_status_label(window)
...
def present_unlock_dialog(window, on_success=None) -> None:
    _start_recording_unlock(window, on_success=on_success)
...
def _start_recording_unlock(window, on_success=None) -> None:
    ...
    if window.demo_mode:
        _show_unlock_error_dialog(window, "Capture unlock is unavailable in demo mode.")
        return
...
def _on_unlock_finished(
    ...
    if success:
        ...
        if lease_id:
            ...
            window._recording_claim_attempt_key = None
            _update_unlock_state(window, claim_response)
        else:
            _update_unlock_state(window, None)
            _request_recording_refresh_lease(window)
        if callable(on_success):
            on_success()
        return False

    _show_unlock_error_dialog(window, error_msg or "Authorization failed")
    return False
...
def _refresh_unlock_lease(window) -> bool:
    ...
    _runtime.session_request_async(
        ...
        lambda result: _on_refresh_unlock_finished(window, result),
        timeout=3.0,
    )
    return True
...
def _on_refresh_unlock_finished(window, result: dict | None) -> bool:
    ...
    if result and result.get("status") == "ok":
        ...
        _update_unlock_state(window, result)
        return False
...
def _request_recording_refresh_lease(window) -> None:
    ...
    _runtime.session_request_async(
        {"command": "claim_recording_unlock_refresh"},
        lambda response: _on_claim_recording_refresh_lease_finished(window, response),
        timeout=3.0,
        on_done=lambda: _on_claim_recording_refresh_lease_done(window),
    )
...
def _on_claim_recording_refresh_lease_finished(window, response: dict | None) -> bool:
    ...
    _update_unlock_state(window, response)
    window._recording_claim_attempt_key = None
    return False
