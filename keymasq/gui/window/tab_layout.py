...
def _iter_tab_children(window):
    for page in window._iter_tab_pages():
...
def _iter_profile_tabs(window):
    yield from window._iter_tab_children()
...
def _append_tab_page(
    ...
    *,
    ...
    page.set_icon(window._icon_from_name(icon_name))
    _runtime.GLib.idle_add(window._sync_tab_close_tooltips)
    return page
...
def _walk_widget_tree(window, widget: _runtime.Gtk.Widget):
    ...
    while child is not None:
        yield from window._walk_widget_tree(child)
        child = child.get_next_sibling()


def _sync_tab_close_tooltips(window) -> bool:
    for widget in window._walk_widget_tree(window.tab_bar):
...
def _hide_combo_tab(window, page: _runtime.Adw.TabPage) -> None:
    window._tab_order = window._current_tab_order()
...
def _current_tab_order(window) -> list[str]:
    order: list[str] = []
    for page in window._iter_tab_pages():
        tab_id = window._tab_id_for_child(page.get_child())
...
def _save_tab_layout(window) -> None:
    window._tab_order = window._merge_hidden_tabs_into_order(window._current_tab_order())
    save_tab_layout(window._tab_order, window._hidden_tabs)
...
def _default_selected_tab_page(window) -> _runtime.Adw.TabPage | None:
    for tab_id in window._desired_visible_tab_order():
        page = window._page_for_tab_id(tab_id)
        if page is not None:
            return page

    for page in window._iter_tab_pages():
        if window._tab_id_for_child(page.get_child()) is not None:
            return page
    return None


def _select_saved_or_default_tab(window) -> None:
    page = window._page_for_selected_tab(window._selected_tab)
    if page is None:
        page = window._default_selected_tab_page()
    if page is None:
        window._initial_tab_selection_pending = False
        return

    window.tab_view.set_selected_page(page)
    selected_tab = window._selected_tab_for_child(page.get_child())
...
def _default_visible_tab_order(window) -> list[str]:
    device_ids: list[str] = []
    other_ids: list[str] = []
    for tab_id in window._current_tab_order():
        if tab_id.startswith(_DEVICE_TAB_PREFIX):
...
def _desired_visible_tab_order(window) -> list[str]:
    current_order = window._current_tab_order()
    if not window._tab_order:
        return window._default_visible_tab_order()
...
def _reorder_visible_pages_to_saved_order(window) -> None:
    desired_order = window._desired_visible_tab_order()
...
def _on_selected_tab_changed(window, _tab_view, _pspec) -> None:
    ...
    selected_tab = window._selected_tab_for_child(child)
    if (
