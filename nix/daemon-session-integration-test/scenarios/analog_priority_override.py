...
OVERRIDE_PROFILE = "Integration Analog Override"


def run(ctx: ScenarioContext) -> None:
    try:
        ctx.set_profile_enabled(PROFILE, enabled=True)
        ctx.set_profile_enabled(OVERRIDE_PROFILE, enabled=True)

        ctx.source_abs(evdev.ecodes.ABS_X, 16384)
        ctx.expect_no_gamepad_events()

        ctx.set_profile_enabled(OVERRIDE_PROFILE, enabled=False)
