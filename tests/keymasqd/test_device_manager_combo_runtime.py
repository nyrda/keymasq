...
class TestCombos:
    ...
    @pytest.mark.asyncio
    async def test_runtime_wheel_pulse_overload_runs_release_actions_immediately(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        manager = DeviceManager()
        ...
        await manager.set_combos(
            [
                {
                    "id": "combo-wheel-overload-down-up",
                    "name": "Wheel Overload Down Up",
                    "steps": [
                        {
                            "events": [
                                {
    ...
    @pytest.mark.asyncio
    async def test_runtime_combo_refresh_preserves_overload_release_actions_until_release(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        manager = DeviceManager()
        ...
        combo_payload = {
            ...
            "steps": [
                {
                    "events": [
                        {"hardware_id": hardware_id, "source": "kbd", "evdev": "ctrl"},
                        {"hardware_id": hardware_id, "source": "kbd", "evdev": "alt"},
        ...
        async def broadcast_callback(command: CommandType, _data: dict[str, object]) -> None:
            if command == CommandType.ACTION_TRIGGER:
        ...
        for code in (evdev.ecodes.KEY_LEFTCTRL, evdev.ecodes.KEY_LEFTALT, evdev.ecodes.KEY_1):
            await _runtime_process_grabbed_event(
                device,
                SimpleNamespace(type=evdev.ecodes.EV_KEY, code=code, value=1),
            )
    ...
    @pytest.mark.asyncio
    async def test_runtime_keyboard_combo_overload_runs_release_actions_only_on_release(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        manager = DeviceManager()
